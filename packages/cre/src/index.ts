...
import { AIAMLAdapter } from "./adapters/aiAmlAdapter";
...
// AML adapter — priority: AI Oracle > Chainalysis > Mock
const amlAdapter = (ETHERSCAN_KEY && GEMINI_KEY)
  ? new AIAMLAdapter(ETHERSCAN_KEY, GEMINI_KEY)
  : CHAINALYSIS_KEY
...
app.post("/api/v1/compliance/screen", async (req: Request, res: Response) => {
  ...
  try {
    ...
    log.aiNarrative = (amlResult as any).aiNarrative ?? null;
    ...
    return res.json({
      ...
      aiNarrative: (amlResult as any).aiNarrative ?? null,
    });
  } catch (err: any) {
    log.status = "FAILED";
    removePending(log.id);
